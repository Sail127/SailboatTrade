...
import { deleteListingR2Assets } from "@/lib/adminListings";
...
export async function deleteUserCompletely(userId) {
  ...
  const user = await prisma.user.findUnique({
    ...
    select: { id: true, email: true, brokerHeroImageUrl: true },
  });
  ...
  await prisma.$transaction([
    ...
    prisma.listing.updateMany({
      where: { removedById: user.id },
      data: { removedById: null },
    }),
    prisma.listing.updateMany({
      where: { contentReviewedById: user.id },
    ...
    ...(ids.length
      ...
      : []),
    prisma.emailDeliveryEvent.deleteMany({
      where: {
        recipient: user.email,
      },
    }),
    ...(ids.length
      ? [prisma.adminAuditLog.deleteMany({ where: { entityType: "Listing", entityId: { in: ids } } })]
      : []),
  ...
  return {
    ...
    deletedListingCount: ids.length,
    deletedAssetCount: assetKeys.filter(Boolean).length,
  };
}
