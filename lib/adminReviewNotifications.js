...
import {
  ...
  buildOwnerListingSoldConfirmationMessage,
  ...
  buildOwnerAutoRenewCanceledMessage,
...
export async function notifyOwnerAutoRenewCanceled({
  req,
  listingId,
  source = "unknown",
}) {
  try {
    ...
    const dashboardUrl = `${appUrl}/dashboard/listings`;
    const listingUrl = `${appUrl}/listings/${encodeURIComponent(id)}`;
    const currentPeriodEndLabel = listing.billingCurrentPeriodEnd
      ? formatDateLong(listing.billingCurrentPeriodEnd)
      : "";

    const message = buildOwnerAutoRenewCanceledMessage({
      appUrl,
      ownerName,
      listingTitle,
      listingId: id,
    ...
    await sendEmailWithRetry({
      ...
      html: message.html,
      text: message.text,
      tags: [{ name: "type", value: "listing_owner_auto_renew_canceled" }],
    });

    return { ok: true };
  } catch (err) {
    console.error("notifyOwnerAutoRenewCanceled failed", {
      listingId: String(listingId || ""),
      source: String(source || ""),
      error: err?.message || String(err || "unknown_error"),
    });
...
export async function notifyOwnerListingSoldConfirmation({
  req,
  listingId,
  soldOnSailboatTrade = false,
  source = "unknown",
  ...
  try {
    ...
    const message = buildOwnerListingSoldConfirmationMessage({
      appUrl,
      ownerName,
      listingTitle,
      soldOnSailboatTrade: Boolean(soldOnSailboatTrade),
    ...
    await sendEmailWithRetry({
      ...
      html: message.html,
      text: message.text,
      tags: [{ name: "type", value: "listing_sold_confirmation" }],
    });

    return { ok: true };
  } catch (err) {
    console.error("notifyOwnerListingSoldConfirmation failed", {
      listingId: String(listingId || ""),
      source: String(source || ""),
      soldOnSailboatTrade: Boolean(soldOnSailboatTrade),
      error: err?.message || String(err || "unknown_error"),
