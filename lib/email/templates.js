...
function sectionBlock({ label, body }) {
  return `
    <div style="margin:0 0 18px;padding-top:18px;border-top:1px solid ${BRAND.line};">
      ...
      <div class="sbt-summary-copy" style="font-size:15px;line-height:1.65;color:${BRAND.ink};">
        ${body}
      </div>
    </div>
  `;
...
function brandWordmark() {
  return `
    <span class="sbt-brand-mark" style="font-family:${BRAND_FONT};font-weight:700;letter-spacing:.03em;">
...
<html>
  ...
  <body style="margin:0;background:${BRAND.shell};font-family:${BODY_FONT};color:${BRAND.ink};">
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="sbt-outer" style="background:${BRAND.shell};padding:24px 12px;">
      <tr>
        <td align="center">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="sbt-shell" style="max-width:640px;background:${BRAND.white};border-radius:20px;overflow:hidden;border:1px solid #dbe5ea;">
            <tr>
              <td class="sbt-pad sbt-header-pad" style="padding:20px 24px;background:${BRAND.navy};border-bottom:4px solid ${BRAND.gold};">
                <table role="presentation" cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="vertical-align:middle;padding-right:12px;">
                      <img src="${esc(assets.logoUrl)}" width="64" alt="SailboatTrade burgee" class="sbt-brand-logo" style="display:block;border:0;width:64px;max-width:64px;height:auto;" />
                    </td>
                    <td style="vertical-align:middle;">
                      <div class="sbt-brand-title" style="font-size:26px;line-height:1.1;">${brandWordmark()}</div>
                      <div class="sbt-kicker" style="margin-top:4px;font-size:12px;font-weight:600;color:#e9f0f4;">All Sailboats, All the Time!</div>
                    </td>
                  </tr>
                </table>
              </td>
            ...
            <tr>
              <td class="sbt-pad" style="padding:28px 24px 26px;">
                ...
                ${
                  intro
                    ? `<p class="sbt-copy" style="margin:0 0 20px;font-size:16px;line-height:1.7;color:${BRAND.ink};">${esc(intro)}</p>`
                    : ""
                }
                ${summaryHtml}
                ${sectionsHtml}
...
export function buildOwnerListingUpgradeConfirmationMessage({
  appUrl,
  ownerName,
  listingTitle,
  listingId,
  ...
  return createMessage({
    appUrl,
    subject: "Thank you for upgrading your SailboatTrade listing",
    preheader: "Your listing upgrade has been confirmed.",
    eyebrow: "Upgrade Confirmed",
    headline: "Thank you for your upgrade",
    intro: `Hi ${personName({ displayName: ownerName })}, thank you for upgrading your listing on SailboatTrade.`,
    heroTitle: "Upgrade receipt",
    heroBody: `<strong>${esc(listingTitle)}</strong><br />Listing ID: ${esc(listingId)}<br />${esc(checkoutLabel)}<br />${esc(totalLabel)}`,
    sections: [
      sectionBlock({
        label: "Upgrade details",
        body: esc(upgradeSummary),
      }),
      sectionBlock({
        label: "Listing status",
        body: esc(statusSummary),
      }),
      sectionBlock({
        label: "Source",
        body: esc(source),
    ...
    primaryCta: { label: "Open Dashboard", href: dashboardUrl, tone: "gold" },
    secondaryCta: { label: "View Listing", href: listingUrl, tone: "navy" },
    textLines: [
      "Thank you for upgrading your SailboatTrade listing",
      `Listing: ${listingTitle}`,
      `Listing ID: ${listingId}`,
      checkoutLabel,
      totalLabel,
      `Upgrade details: ${upgradeSummary}`,
      `Listing status: ${statusSummary}`,
      `Dashboard: ${dashboardUrl}`,
      `View listing: ${listingUrl}`,
      `Source: ${source}`,
    ],
...
export function buildOwnerAutoRenewCanceledMessage({
  appUrl,
  ownerName,
  listingTitle,
  listingId,
  ...
  return createMessage({
    appUrl,
    subject: "Auto-renew is off for your SailboatTrade listing",
    preheader: "Future PayPal charges have been stopped for this listing.",
    eyebrow: "Auto-Renew Off",
    headline: "Auto-renew has been turned off",
    intro: `Hi ${personName({ displayName: ownerName })}, future recurring PayPal charges have been stopped for this listing.`,
    heroTitle: "Listing",
    heroBody: `<strong>${esc(listingTitle)}</strong><br />Listing ID: ${esc(listingId)}${
      currentPeriodEndLabel ? `<br />Current paid term ends: ${esc(currentPeriodEndLabel)}` : ""
    }`,
    sections: [
      sectionBlock({
        label: "What this means",
        body: currentPeriodEndLabel
          ? `Your current paid term stays active until ${esc(currentPeriodEndLabel)}. After that, the listing will not renew automatically unless you start a new paid term.`
          : "Your current paid term stays active for the rest of its already-paid billing cycle. After that, the listing will not renew automatically unless you start a new paid term.",
      }),
      sectionBlock({
        label: "Source",
        body: esc(source),
    ...
    textLines: [
      ...
      `Listing: ${listingTitle}`,
      `Listing ID: ${listingId}`,
      currentPeriodEndLabel ? `Current paid term ends: ${currentPeriodEndLabel}` : null,
      "Future PayPal charges have been stopped for this listing.",
      `Dashboard: ${dashboardUrl}`,
      `View listing: ${listingUrl}`,
      `Source: ${source}`,
    ].filter(Boolean),
...
export function buildAdminListingSoldReportMessage({
  appUrl,
  ownerName,
  ownerEmail,
  listingTitle,
  ...
  const soldLabel = soldOnSailboatTrade ? "Yes" : "No";
  return createMessage({
    appUrl,
    subject: `${soldOnSailboatTrade ? "ST sale" : "Sold elsewhere"}: ${listingTitle}`,
    preheader: "A seller reported that their listing has sold.",
    eyebrow: "Listing Sold Report",
    headline: "A seller reported a sold boat",
    intro: `The owner of <strong>${esc(listingTitle)}</strong> marked the listing as sold.`,
    heroTitle: "Sale Summary",
    heroBody: [
      `<strong>Listing:</strong> ${esc(listingTitle)}`,
      `<strong>Owner:</strong> ${esc(ownerName)} (${esc(ownerEmail)})`,
      `<strong>Sold on SailboatTrade.com:</strong> ${esc(soldLabel)}`,
      `<strong>Listing ID:</strong> ${esc(listingId)}`,
    ].join("<br />"),
    sections: [
      sectionBlock({
        label: "Seller Feedback",
        body: feedback ? plainTextToHtml(feedback) : "No additional feedback was provided.",
      }),
    ],
    primaryCta: { label: "Open Dashboard", href: dashboardUrl, tone: "gold" },
    textLines: [
      "A seller reported a sold boat",
      `Listing: ${listingTitle}`,
      `Owner: ${ownerName} (${ownerEmail})`,
      `Sold on SailboatTrade.com: ${soldLabel}`,
      `Listing ID: ${listingId}`,
...
export function buildOwnerListingSoldConfirmationMessage({
  appUrl,
  ownerName,
  listingTitle,
  soldOnSailboatTrade,
  ...
  return createMessage({
    appUrl,
    subject: "Thank you for listing with SailboatTrade",
    preheader: "Your sold listing has been archived and is no longer public.",
    eyebrow: "Listing Sold",
    headline: "Thank you for listing with us",
    intro: `Hi ${personName({ displayName: ownerName })}, thanks for letting us know your boat sold.`,
    heroTitle: "Listing Update",
    heroBody: [
      `<strong>${esc(listingTitle)}</strong>`,
      "This listing is now inactive and archived.",
    ].join("<br />"),
    sections: [
      sectionBlock({
        label: "Archive Confirmation",
        body:
          "Your listing has been removed from public search results and archived in accordance with SailboatTrade storage policies. Photos remain for 30 days, then only the hero image is retained.",
      }),
      sectionBlock({
        label: "Sale Source",
        body: `Sold as a result of SailboatTrade.com advertisement: ${esc(soldLabel)}`,
      }),
    ],
    primaryCta: { label: "Open Dashboard", href: dashboardUrl, tone: "gold" },
    textLines: [
      "Thank you for listing with SailboatTrade",
      `Listing: ${listingTitle}`,
      "This listing is now inactive and archived.",
      "Archived listings follow SailboatTrade storage policies. Photos remain for 30 days, then only the hero image is retained.",
      `Sold as a result of SailboatTrade.com advertisement: ${soldLabel}`,
      `Dashboard: ${dashboardUrl}`,
    ],
  });
}
...
export function getEmailPreviewCatalog({ appUrl, currentAdminEmail, currentAdminName }) {
  ...
  return [
    ...
    {
      key: "admin_listing_sold_report",
      group: "Admin",
      label: "Listing Sold Report",
      description: "Staff summary when a seller marks a listing as sold.",
      previewTo: currentAdminEmail || "support@sailboattrade.com",
      ...buildAdminListingSoldReportMessage({
        appUrl: base,
        ownerName: "Robin Wright",
        ownerEmail: "robin@wrighthouse.net",
        listingTitle: "2016 Jeanneau 403",
        listingId: sampleListingId,
        soldOnSailboatTrade: true,
        feedback: "Lead quality felt strong and the inquiry flow was easy to use. A little more buyer profile detail would help.",
        dashboardUrl: sampleDashboardUrl,
      }),
    },
  ];
