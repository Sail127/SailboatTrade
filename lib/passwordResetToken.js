...
function signingKeyForUser(passwordHash) {
  const base = getSecret();
  const hashPart = String(passwordHash || "").trim();
  if (!hashPart) throw new Error("Missing password hash for reset token signing.");
  return `${base}:${hashPart}`;
}

export function createResetToken({ email, passwordHash, ttlMinutes = 30 }) {
  const exp = Date.now() + ttlMinutes * 60 * 1000;
  const payload = { email, exp, nonce: crypto.randomBytes(8).toString("hex") };
  const payloadB64 = b64urlEncode(JSON.stringify(payload));

  const sig = crypto
    .createHmac("sha256", signingKeyForUser(passwordHash))
    .update(payloadB64)
    .digest();
...
export function peekResetTokenEmail(token) {
  if (!token || typeof token !== "string") return null;
